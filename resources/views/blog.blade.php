            ...
            @foreach ($posts as $post)
                <article class="w-full border-2 rounded-lg">
                    <div class="p-2">
                        ...
                        <p class="text-xs font-light">By. <a href="/author/{{ $post->user->username }}" class="underline">{{ $post->user->name }}</a> in <a href="/categories/{{ $post->category->slug }}" class="underline">{{ $post->category->name }}</a></p>

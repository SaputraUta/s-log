...
@section('container')
    <article class="mx-56">
        ...
        <p>By. <a href="/blog?user={{ $post->user->username }}" class="underline">{{ $post->user->name }}</a> in <a
                href="/blog?category={{ $post->category->slug }}" class="underline">{{ $post->category->name }}</a></p>
        @if ($post->image)
